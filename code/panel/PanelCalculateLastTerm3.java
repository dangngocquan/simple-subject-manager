...
public class PanelCalculateLastTerm3 extends JPanel {
    ...
    public PanelCalculateLastTerm3(int x, int y, int width, int height, Font font, ConversionTable convertionTable,
        ...
        panelScore2 = new PanelString(0, panelScore1.getY() + panelScore1.getHeight() + 10, "Điểm thứ hai: ", width / 8,
                font,
                PanelString.TOP_LEFT, 15);
        textField1 = new TextField(panelScore1.getWidth(), panelScore1.getY(), width / 10, panelScore1.getHeight(),
                TextField.TOP_LEFT, "9.04", 2, 15, 15);
        textField2 = new TextField(panelScore2.getWidth(), panelScore2.getY(), width / 10, panelScore2.getHeight(),
                TextField.TOP_LEFT, "9.04", 2, 15, 15);
        panelCoefficient1 = new PanelString(textField1.getX() + textField1.getWidth() + width / 15, panelScore1.getY(),
                "Hệ số của điểm thứ nhất (%): ", width / 5 + 30, font, PanelString.TOP_LEFT, 15);
        panelCoefficient2 = new PanelString(textField2.getX() + textField2.getWidth() + width / 15, panelScore2.getY(),
                "Hệ số của điểm thứ hai (%): ", width / 5 + 30, font, PanelString.TOP_LEFT, 15);
        textField3 = new TextField(panelCoefficient1.getX() + panelCoefficient1.getWidth(), panelCoefficient1.getY(),
                width / 10, panelCoefficient1.getHeight(),
                TextField.TOP_LEFT, "20", 2, 15, 15);
        textField4 = new TextField(panelCoefficient2.getX() + panelCoefficient2.getWidth(), panelCoefficient2.getY(),
                width / 10, panelCoefficient2.getHeight(),
                TextField.TOP_LEFT, "20", 2, 15, 15);

        buttonSubmit = new Button("Tính toán");
        buttonSubmit.setSizeButton(width / 10, panelScore2.getY() + panelScore2.getHeight() - panelScore1.getY(), true);
    ...
    public boolean isValidInput() {
        if (!textField1.getText().matches("[0-9]{1,}") && !textField1.getText().matches("[0-9]{1,}\\.[0-9]{1,}")) {
            ...
            JOptionPane.showMessageDialog(applicationFrame, "Hệ số của điểm thứ nhất đang lớn hơn 100%, hư cấu",
                    "Invalid input",
                    JOptionPane.WARNING_MESSAGE);
            return false;
            ...
            JOptionPane.showMessageDialog(applicationFrame, "Hệ số của điểm thứ hai đang lớn hơn 100%, hư cấu",
                    "Invalid input",
                    JOptionPane.WARNING_MESSAGE);
            return false;
        } else if (Double.parseDouble(textField3.getText()) + Double.parseDouble(textField4.getText()) >= 100) {
            JOptionPane.showMessageDialog(applicationFrame, "Tổng hệ số của điểm thứ nhất và thứ hai đang không hợp lý",
                    "Invalid input",
                    JOptionPane.WARNING_MESSAGE);
            return false;
        }
    ...
    private class ButtonHandler implements ActionListener {
        public void actionPerformed(ActionEvent event) {
            if (event.getSource() == buttonSubmit) {
                if (isValidInput()) {
                    DialogCalculateScoreLastTerm3 dialog = new DialogCalculateScoreLastTerm3(Setting.WIDTH / 2,
                            Setting.HEIGHT / 2, Setting.WIDTH / 5 * 4, Setting.HEIGHT / 5 * 4,
