...
public class DialogInput {
    ...
    private String[] textDefaults = null;

    // Constructor
    public DialogInput(int x, int y, int width, int height, int rootLocationType, String title,
            String[] messageLines, ImageIcon icon, String[] textDefaults) {
        this.textDefaults = textDefaults;
        this.textFields = new TextField[textDefaults.length];
        ...
        for (int i = 0; i < textFields.length; i++) {
            textFields[i] = new TextField(messagePanel.getX(), tempHeight, messagePanel.getWidth(),
                    buttonIcon.getHeight(), TextField.TOP_LEFT,
                    this.textDefaults[i], 2, 10, 10);
            dialog.add(textFields[i]);
            tempHeight += textFields[i].getHeight() + 10;
        }
