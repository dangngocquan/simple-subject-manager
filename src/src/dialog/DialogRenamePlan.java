...
public class DialogRenamePlan {
    ...
    // Constructor
    public DialogRenamePlan(int x, int y, int width, int height, int rootLocationType, String title,
        ...
        fieldName = new TextField(width / 2, tempHeight,
                width / 5 * 3, 50, TextField.TOP_CENTER, "Tên mới", 2, 15, 15);
            ...
            if (event.getSource() == button) {
                if (fieldName.getText().isEmpty() ||
                    ...
                    new DialogMessage(Setting.WIDTH / 2, Setting.HEIGHT / 2, Setting.WIDTH / 3, Setting.HEIGHT / 3,
                            ...
                            "Information", new String[] { "Bạn chưa nhập tên mới" }, Setting.WARNING);
                } else {
                    List<Plan> plans = ReadFile.getCurrentAccount().getPlans();
                    WriteFile.renamePlan(plans.get(indexPlan), indexPlan, fieldName.getText());
                    new DialogMessage(Setting.WIDTH / 2, Setting.HEIGHT / 2, Setting.WIDTH / 3, Setting.HEIGHT / 3,
                            DialogMessage.CENTER_CENTER,
                            "Information", new String[] { "Đổi tên kế hoạch thành công" }, Setting.INFORMATION);
                    dialog.dispose();
                }
            }
        }
